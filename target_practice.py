...
list_of_years = ['2011', '2012', '2013', '2014', '2015', '2016', '2017', '2018', '2019']


def get_database(data, years):
    database = {}
    for elem in data:
        country = elem[0]
        ...
        for index, coverage in enumerate(elem[1]):
            new_object = {'year': years[index], 'coverage': coverage}

            database[country].append(new_object)

    return database

def year_index(given_year, data):
    year_data = []
    for country in data:
        for entry in data[country]:
            if int(entry['year']) == given_year:
                entry["country"] = country
                year_data.append(entry)
    return year_data

def country_index(given_country, data):
    return data[given_country]
...
def average(given_list):
    ...
    for entry in given_list:
        if entry["coverage"] != ": ":
            if entry["coverage"][-1] == "b":
                sum += int(entry["coverage"][0:-1])
            else:
                sum += int(entry["coverage"])

    return sum/len(given_list)

def main():
    result = get_database(raw_data, list_of_years)
    pp = pprint.PrettyPrinter(indent=4)
    pp.pprint(result)
    print('------------------------')
    for country in year_index(2015, result):
        print(country)
    print(country_index('RO', result))

    print(average(country_index('CH', result)))
    print(average(year_index(2015, result)))
